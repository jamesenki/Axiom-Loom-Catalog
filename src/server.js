    ...
    const repos = fs.readdirSync(reposPath, { withFileTypes: true })
      ...
      .map(dirent => {
        ...
        return {
          id: dirent.name,
          name: dirent.name,
          displayName: portalMetadata?.name || repo.displayName || metadata.displayName || friendlyName,
...
// Health check endpoints (no auth required) - for container orchestration
// Liveness probe - checks if the service is alive
app.get('/health', (req, res) => {
  res.status(200).json({
    status: 'healthy',
    service: 'axiom-loom-catalog-api',
...
app.get('/ready', (req, res) => {
  ...
  const checks = {
    repositoriesAvailable: fs.existsSync(reposPath),
    memoryUsage: process.memoryUsage(),
    uptime: process.uptime()
  };

  const isReady = checks.repositoriesAvailable;
...
// Legacy health check endpoint (no auth required) - keep for backwards compatibility
